...
int translate(const char *in_file_path, const char *segmentless_file_path ,const char *out_file_path, const char *array_name){
    std::cout << std::endl << "Translating: " << std::endl;
    std::ifstream in(in_file_path);
    std::ifstream sl(segmentless_file_path);
    ...
    while((getline_s(sl, comment), getline_s(in, line))){
        std::vector<std::string> tokens;
        line_token_extract(line, tokens);
        if(!line_translator(tokens, line, "")) {
            std::sprintf(machine_code, "%s[%3d] = 0b%s;", array_name, line_idx, line.c_str());
            out << machine_code << "  //" << comment << std::endl;
            line_translator(tokens, line, " ");
            std::printf("%s[%3d] = 0b%s;\n", array_name, line_idx, line.c_str());
            line_idx++;
        }
        else{
